...
/// Backup all data from directx context.
#[derive(Default)]
// TODO: This was found with a RefCell... Maybe this is required to stop it from being dumb?
pub struct BackupState(InnerState);
...
impl BackupState {
    ...
    #[inline]
    pub fn restore(&mut self, context: &ID3D11DeviceContext) {
        unsafe {
            self.0.restore(context);
        }
    }
}
