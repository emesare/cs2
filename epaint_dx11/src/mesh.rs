use epaint::{Mesh, Pos2, Rect, Rgba, TextureId, Vertex};
use std::mem::size_of;
...
pub fn create_vertex_buffer(
    device: &ID3D11Device,
    mesh: &GpuMesh,
) -> Result<ID3D11Buffer, DirectX11Error> {
    ...
    unsafe {
        ...
        output.ok_or(DirectX11Error::General("Failed to create vertex buffer"))
    }
}
...
pub fn create_index_buffer(
    ...
    unsafe {
        let mut output = None;
        device.CreateBuffer(&desc, Some(&init), Some(&mut output))?;
        output.ok_or(DirectX11Error::General("Failed to create index buffer"))
    }
}
