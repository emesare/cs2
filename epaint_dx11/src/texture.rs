use epaint::{ahash::HashMap, textures::TexturesDelta, Color32, ImageData, TextureId};
...
impl TextureAllocator {
    ...
    fn update_partial(
        ...
        if let Some(old) = self.allocated.get_mut(&tid) {
            ...
            match image {
                ...
                _ => unreachable!(), // TODO: We dont support partial update ImageData::Color
            }
    ...
    fn allocate_texture(
        dev: &ID3D11Device,
        image: ImageData,
    ) -> Result<ManagedTexture, DirectX11Error> {
        ...
        let pixels = match image {
            ImageData::Color(c) => c.pixels.to_owned(),
            ImageData::Font(f) => f
                .pixels
                .iter()
                .map(|a| Color32::from_rgba_premultiplied(255, 255, 255, (a * 255.) as u8))
        ...
        let data = D3D11_SUBRESOURCE_DATA {
            ...
            SysMemPitch: (img_width * size_of::<Color32>()) as u32,
            SysMemSlicePitch: 0,
        };
        ...
        unsafe {
            let texture = {
                ...
                output_texture.ok_or(DirectX11Error::General("Unable to create Texture 2D"))?
            };
            ...
            let resource = {
                ...
                output_resource.ok_or(DirectX11Error::General("Failed to create shader view"))?
            };
